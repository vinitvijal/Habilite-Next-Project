...
function Blog(){
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-6 ">
      ...
      <h1 className="text-xl font-semibold text-gray-500 mb-20 pb-4 text-center">Here's what we've been to lately </h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 m-3">
        {blogs.map((blog) => (
          <div key={blog.id} className="bg-first shadow-md rounded-lg overflow-hidden hover:shadow-lg hover:scale-105 transition-transform duration-300 hover:bg-blue-500">
            <img src={blog.imageUrl} alt={blog.title} className="w-full h-52 rounded-xl object-cover p-2 " />
            <div className="p-4">

              <h2 className="mt-2 text-lg font-bold text-white">
                {blog.title}
              </h2>
              <p className="text-sm text-white mt-2">{blog.excerpt}</p>
      ...
      <div className="w-full flex justify-center mt-12">
          <Link href="/blogs" className="px-4 py-2 bg-first rounded-lg hover:scale-105 duration-300 hover:duration-500 text-white hover:bg-second">
            More Blogs
          </Link>
      </div>
    </div>
