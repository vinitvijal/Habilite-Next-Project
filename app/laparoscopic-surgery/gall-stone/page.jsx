...
const FeatureList = ({ features }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 text-first text-lg text-left mx-auto">
    {features.map((feature, index) => (
      <div
        key={index}
        className={`flex items-start space-x-2 py-3 ${index % 2 === 0 ? "md:mr-4" : "md:ml-4"} ${
          index < features.length - 2 ? "border-third border-b" : ""
        }`} >
        <span>➔</span>
        <p>{feature}</p>
      </div>
  ...
  const featureData = [
    {
      ...
      icon: (<svg viewBox="0 0 55 44" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-white">
      <path
...
'>
  ...
  <div className="absolute inset-0 flex items-center justify-center">
    <h1 className="text-7xl font-bold text-white px-4 py-2 rounded-md flex-col items-center justify-center text-center text-fourth ">
      Best Gallbladder Surgeon <br /> in Delhi
        ...
        <motion.div
              ...
              whileInView="visible"
              viewport={{ amount: 0.2 }}
              className="w-3/5 mx-auto relative -mt-16 px-14 py-10 bg-white shadow-sm shadow-black items-center rounded-md"
            >
  ...
  {surgeonInfo.map((info, index) => (
    <div key={index} className="mb-8">
      ...
      <motion.p
        ...
        viewport={{ amount: 0.2 }}
        className="text-xl mb-8 text-first max-w-3xl mx-auto"
      >
      ...
      transition={{ duration: 2 }}
      viewport={{ once: false }}  className='py-8 px-32 my-8 text-center border-t-2 border-b-2 border-third '>
      ...
      <div className="grid md:grid-cols-2 gap-16 max-w-7xl mx-auto p-10">
        {featureData.map((feature, index) => (
          <motion.div
            key={index}
            className="rounded-xl bg-white p-12 shadow-md border-third border-2"
            variants={cardVariants1}
            initial="hidden"
            whileInView="visible"
            viewport={{ amount: 0.2 }}
          >
            <div className="mx-auto flex items-center justify-center h-24 w-24 -translate-y-16 transform rounded-full shadow-lg bg-first">
              <img src={feature.icon} alt={feature.title} className="h-12 w-12 text-white" />
            </div>
      ...
      transition={{ duration: 2 }} 
      viewport={{ amount: 0.2 }}  className='flex-col mx-6 mb-16 pt-4 px-24 border-third border-t-2'>
